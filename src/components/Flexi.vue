...
<template>
  <section class="py-10 section" :style="{ backgroundColor: background }">
    <div class="container px-4 lg:flex">
      ...
      <div class="relative w-full lg:w-5/12 image" :class="{
      ...
      <div class="hidden lg:block w-2/12 order-2"></div>

      <div class="w-full lg:w-5/12 content"
      :class="{
        'order-1': side === 'image_right',
        'order-3': side === 'image_left'
      }">
        <h4 :class="{'hidden': subtitle === '' }">{{ subtitle }}</h4>
        <h2 class="leading-none mb-8">{{ title }}</h2>
        <p v-html="para"></p>
      </div>
    </div>
  </section>
